  ...
  "SensitiveAddress" should {
    ...
    "encrypt and decrypt address fields correctly" in {
      val originalRequestRefDetails = RequestReferenceNumberDetails(
        ...
        requestReferenceContactDetails = Some(
          RequestReferenceNumberContactDetails(
            requestReferenceNumberContactDetailsFullName = "Full Name",
            requestReferenceNumberContactDetails = ContactDetails(
              phone = "0123456789",
              email = "test@email.com"
            ),
            requestReferenceNumberContactDetailsAdditionalInformation = Some("Additional information")
          )
        )
      )

      val sensitiveRequestReferenceNumber = SensitiveRequestReferenceNumber(originalRequestRefDetails)

      // Ensure the sensitive fields are encrypted
      sensitiveRequestReferenceNumber.requestReferenceNumberAddress.get.noReferenceNumberBusinessTradingName
        .isInstanceOf[SensitiveString] shouldBe true
      sensitiveRequestReferenceNumber.requestReferenceNumberAddress.get.noReferenceNumberAddress.buildingNameNumber
        .isInstanceOf[SensitiveString] shouldBe true
      sensitiveRequestReferenceNumber.requestReferenceNumberAddress.get.noReferenceNumberAddress.town
        .isInstanceOf[SensitiveString] shouldBe true
      sensitiveRequestReferenceNumber.requestReferenceNumberAddress.get.noReferenceNumberAddress.postcode
        .isInstanceOf[SensitiveString] shouldBe true
