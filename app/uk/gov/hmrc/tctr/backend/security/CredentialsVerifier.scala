...
class IPBlockingCredentialsVerifier @Inject() (
  ...
  private def startOfLoginSession: Instant = clock.now().minusMinutes(config.sessionWindow.toMinutes.toInt).toInstant

  private def lockoutWindow: Instant = clock.now().minusMinutes(config.lockoutWindow.toMinutes.toInt).toInstant
